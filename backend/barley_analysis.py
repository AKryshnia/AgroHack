...
Entrez.email = "svartha@gmail.com"
...
def download_sequences_for_genes(genes, organism, output_file, database="nucleotide", max_results=10):
    sequences = []
    for gene in genes:
        ...
        try:
            ...
            logger.info(f"Найдено {len(ids)} записей для {gene}. Скачиваем данные...")
...
# Функция для идентификации мутаций
...
def reconstruct_morphology(mutations):
    # Здесь вы можете использовать научную литературу и информацию о функциях генов Btr1, Btr2, Vrs1
    # Для простоты добавим заглушку
    with open(morphology_report_file, "w") as f:
...
def perform_phylogenetic_analysis(alignment):
    ...
    # Сохранение отчета
    with open(region_report_file, "w") as f:
        f.write("Отчет о возможном регионе возделывания древнего ячменя\n\n")
        f.write("На основе филогенетического анализа древний ячмень наиболее близок к...\n")
        f.write("Это может указывать на то, что регионом возделывания является...\n")
...
def main():
    ...
    if not os.path.exists(vulgare_file):
        download_sequences_for_genes(genes, "Hordeum vulgare", vulgare_file, max_results=20)
    
    if not os.path.exists(spontaneum_file):
        download_sequences_for_genes(genes, "Hordeum vulgare subsp. spontaneum", spontaneum_file, max_results=20)
    ...
    with open(os.path.join(output_dir, "amino_acid_changes.txt"), "w") as f:
